...
| `GET /produtos/manifesto` | `response.send(gerarManifesto())` — a text nothing readable flows into                         |
...
| GET /produtos/manifesto    | EO   | 1       | 1   | low        | 4      | 1 opaque document, **reported**; the count was read, not delivered         |
...
6. A call that returns no literal and reads no store — a CSV builder, a
   ...
   1 DET, opaque, reported (`gerarManifesto()`).
...
Predicted 1.5.0 total: **52 FP** (0) — and printed, once, before any code, to
