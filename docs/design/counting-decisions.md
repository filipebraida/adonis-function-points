...
| ace command              | `commandName` — rendered `ace <commandName>`, the way HTTP renders `<verb> <pattern>` (0.7: `commands/**` extending `BaseCommand`)   |
...
| `this.ui.table().row([r.nome, r.email])`, `this.logger.info(\`${n} rows\`)`, `console.log(x)` — an ace command prints | printing is delivery: a field read off a row is one DET named after it, a template's expressions are its values, a count is one; a bare string is a label, not a field (plan 0.7 §C) |
